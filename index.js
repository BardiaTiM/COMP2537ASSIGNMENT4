$(document).ready(function () {
  ...
  let timerId;
  let disableClick = false; // Variable to track the click status
  ...
  const setup = () => {
    ...
    function updateScore() {
      document.getElementById("score").innerHTML = "Number of Matches: " + matches;
    }
    ...
    $(".card").on("click", function () {
      if ($(this).hasClass("flip") || isChecking || disableClick) {
        // Prevent clicking on a flipped card, during card comparison, or when click is disabled
        return;
      }
      ...
      if (!firstCard) {
        ...
        if (firstCard.src == secondCard.src) {
          ...
          setTimeout(() => {
            ...
            // Enable clicking after 4 seconds
            disableClick = true;
            setTimeout(() => {
              disableClick = false;
            }, 4000);
